...
public class E05DateCalendar1 {
	...
	public static String[] myDiffDate() throws ParseException{
		...
		Date formaFuture = new SimpleDateFormat("yyyy/MM/dd").parse(future);
		/*
		getTime()으로 얻어오는 시간은 총 13자리의 정수값인데
		1~10자리까지는 순수한 초이고, 11~13자리까지는 밀리세컨즈를 의미한다.
		즉 1000으로 나눠서 뒤의 3자리를 제거하면 순수한 초가 반환된다.
		 */
		long diffSec = (formaFuture.getTime() - formatToday.getTime()) / 1000; //1초
		long diffMin = (formaFuture.getTime() - formatToday.getTime()) / 60000; //1분
		long diffHor = (formaFuture.getTime() - formatToday.getTime()) / 3600000; //1시간

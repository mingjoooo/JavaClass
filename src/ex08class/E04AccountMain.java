...
class Account{
	...
	//멤버변수
	String name; //예금주
	String accountAccount; //계좌번호
	int balance; //잔고
	
	//멤버메소드 (입금과 출금의 지역이 다르기 때문에 money변수가 동일하게 사용가능하다.
	//입금처리 : 매개변수로 전달되는 금액만큼 입금처리한다.
	void deposit(int money) {
		balance += money; //money만큼 balance(잔고)에 +된다
		System.out.println(money+"원이 입금됨");
	}
	//출금처리 
	void withdrawe(int money) {
		if(balance >= money) {
	...
	void showAccount() {
		...
		System.out.println("계좌번호:"+ accountAccount);
...
public class E04AccountMain {
	...
	public static void main(String[] args) {
		...
		account.withdrawe(5000);
		account.showAccount();
		
		//두번째 통장 개설 : 멤버변수에 직접 접근해서 초기화 후 잔고확인
		Account account2 = new Account();
		account2.name = "정우성";
		account2.accountAccount = "123-45-67890";

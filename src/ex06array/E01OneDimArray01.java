...
public class E01OneDimArray01
{
	...
	public static void main(String[] args)
	{
		...
		배열의 선언 및 초기화
			...
			접근을 위해 일반적으로 반복문(for)을 사용한다.//갯수가 정해져있고
		배열명.length
		...
		int[] iniArr = new int[1];
		iniArr[0] = 100;
		System.out.println("iniArr[0]="+ iniArr[0]);
		
		/*
		배열의 선언과 동시에 초기화를 하는경우 자바 컴파일러는
		초기밧의 갯수를 통해 크기를  결정할 수 있다. 따라서 이런경우
		크기는 생략하도록 약속되어있다.
		 */
		System.out.println("방법2 : 선언과 동시에 초기화");
		//int[] iniArr2 = new int[3] {1, 20, 300};
		int[] iniArray2 = new int[] {1, 20, 300};
		
		/*
		배열의 요소의 갯수를 통해 배열을 생성한 후 초기화까지 동시에
